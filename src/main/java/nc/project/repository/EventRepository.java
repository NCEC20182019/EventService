...
public interface EventRepository extends PagingAndSortingRepository<Event, Integer> {
    ...
    List<Event> findAllByDateStartGreaterThanEqual(Date start);

    List<Event> findAllByDateEndLessThanEqual(Date end);

    @Query(value = "select s.* from events s " +
            "join locations l on " +
            "l.location_id = s.location_id " +
            "where getDistance(l.latitude, l.longitude, :lat, :lng) <= :radius", nativeQuery = true)
